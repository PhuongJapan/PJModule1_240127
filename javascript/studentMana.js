...
function displayDataToEdit(studentId) {
  ...
  if (studentIndexUpdate>=0&&courseIndex>=0&&classIndex>=0) {
    ...
    statusElement.forEach((element)=>{
      if (arrCourse[courseIndex].arrClass[classIndex].arrStudent[studentIndexUpdate].status==element.value) {
    ...
    //   document.getElementById("activeEdit").selected = true;
...
function orderStudentName() {
  ...
  let arrStudentSort = getListStudent(arrCourse);
