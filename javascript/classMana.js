...
function createClass() {
  ...
  if (!validateClassName(newClass.className)) {
    return;
  }
...
function displayDataToEdit(classId) {
  ...
  if (classIndexUpdate >= 0 && courseIndex >= 0) {
    ...
    // }
    let statusElement = document.querySelectorAll("#statusEdit option");
    statusElement.forEach((element)=>{
      if (arrCourse[courseIndex].arrClass[classIndexUpdate].status==element.value) {
...
document
  ...
  .addEventListener("click", function () {
    ...
    let searchArrClass = localStorage.getItem("studentManagement")
      ? JSON.parse(localStorage.getItem("studentManagement"))
      : [];
    // let searchArrCourse = JSON.parse(localStorage.getItem("arrCourse"));
    let filterData = arrClass.filter((item) =>
...
function orderClassName() {
  let arrClassSort = localStorage.getItem("studentManagement")
    ? JSON.parse(localStorage.getItem("studentManagement"))
    : [];
  arrClassSort = arrClass;
